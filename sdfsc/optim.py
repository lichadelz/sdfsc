...
import time
import utils
import sdfsc.colchecker as colchecker
...
def adam_traj_optimize(robot, dist_est, start_cfg, target_cfg, options):
    ...
    def cost(p):
        ...
        obj = (control_points[1:]-control_points[:-1]).pow(2).sum()#+0.5*(p[1:]-p[:-1]).pow(2).sum()
    ...
    for trial_time in range(NUM_RE_TRIALS):
        ...
        for step in range(MAXITER):
            ...
            if loss.data.cpu().numpy() < lowest_loss:
                lowest_loss = loss.data.cpu().numpy()
                lowest_loss_solution = p.data.clone()
                lowest_loss_step = step
                lowest_loss_trial = trial_time
                lowest_loss_obj = objective_loss.data.cpu().numpy()
            if constraint_loss <= 1e-2:
                if objective_loss.data.cpu().numpy() < best_valid_obj:
                    best_valid_obj = objective_loss.data.cpu().numpy()
...
def givengrad_traj_optimize(robot, dist_est, start_cfg, target_cfg, options):
    ...
    def con_collision_free(p, return_tensor=False):
        ...
        if n_point % n_segment != 0:
            multiplier += 1
            if collision_cost.is_cuda:
                device = collision_cost.device
            else:
                device = torch.device("cpu")

            pad_cost = torch.zeros(n_segment*multiplier-n_point, device=device)
        ...
        print("con_collision_free=",collision_cost)
        return collision_cost if return_tensor else collision_cost.data.cpu().numpy()
    ...
    def jac_con_collision_free(p): # Full jacobian, vs only gradient of sum
        ...
        # print("jac_con_collision_free=",jac[:, 1:-1].numpy().reshape(n_constraints, -1))
    ...
    def cost(p):
        ...
        # obj = (var_p_cost[1:]-var_p_cost[:-1]).pow(2).sum()
        return obj.data.numpy()
...
def trustconstr_traj_optimize(robot, dist_est, start_cfg, target_cfg, options):
    ...
    def con_collision_free(p, return_tensor=False):
        ...
        collision_cost=collision_cost.cpu()
    ...
    def cost(p, return_tensor=False):
        ...
        if not return_tensor:
            var_p_cost = pre_process(p)
            print("var_p_cost.shape=",var_p_cost.shape)
            latest_p_cost = var_p_cost.data[1:-1].numpy().reshape(-1)
            control_points = robot.fkine(var_p_cost)
            obj = (control_points[1:]-control_points[:-1]).pow(2).sum()#+(var_p_cost[1:]-var_p_cost[:-1]).pow(2).sum()
    ...
    def grad_cost(p):
        ...
        if not np.allclose(p, latest_p_cost):
            print("not np.allclose(p, latest_p_cost)")
            cost(p)
        if not grad_cost_valid:
            print("not grad_cost_valid:")
            var_p_cost.grad = None
            print("1",var_p_cost.grad)
            obj.backward()
            grad_cost_valid = True
        print("2",var_p_cost.grad)
        if var_p_cost.grad is None:
            print("var_p_cost.grad is None:")
            return np.zeros(len(p), dtype=p.dtype)
        # print("3",var_p_cost.grad)
        return var_p_cost.grad[1:-1].numpy().reshape(-1)
    ...
    for trial_time in range(NUM_RE_TRIALS):
        if trial_time == 0:
            if 'init_solution' in options:
                assert isinstance(options['init_solution'], torch.Tensor)
                #init_path = sparse_path(RRT_path)
                init_path = options['init_solution']
    ...
    solution_rec.x = solution_rec.x.reshape([-1, robot.dof])
    solution_rec.x = pre_process(solution_rec.x)
    print("solution_rec=",solution_rec)
    rec = {
        'start_cfg': start_cfg.numpy().tolist(),
        'target_cfg': target_cfg.numpy().tolist(),
        'cnt_check': cnt_check,
...
class TrajOptimizer:
    def __init__(self, robot, checker, options):
        self.robot = robot
        self.checker: colchecker = checker
